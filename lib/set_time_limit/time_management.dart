...
class TimeManagement extends StatelessWidget {
  final String childId;  // Add childId to focus on the selected child
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(
          'Total Screen Time Limit for Child $childId',  // Display the child ID
          style: TextStyle(
            fontSize: 20.0,
        ...
        TimeLimitWidget(childId: childId),  // Pass the childId to the widget
      ],
    );
  }
}
...
class TimeLimitWidget extends StatefulWidget {
  final String childId;  // Pass the childId here

  const TimeLimitWidget({super.key, required this.childId});
...
class TimeLimitWidgetState extends State<TimeLimitWidget> {
  ...
  Future<void> _saveSchedules() async {
    ...
    // Use widget.childId here
    await dbHelper.saveTimeManagement(schedules, _totalScreenTime, widget.childId);
  }
                ...
                showDialog(
                  ...
                  builder: (BuildContext context) {
                    return ScreenTimeScheduleDialog(
                      ...
                      childId: widget.childId, // Add the childId argument here
                    );
                  },
                );
              },
