...
const dashboardScreenTimeRoutes= require('./dashboard_screen_time');
const dashboardAppTimeRoute = require('./dashboard_app_time');
...
app.get('/', (req, res) => {
  res.send('Welcome to the Famie App! Run your Flutter now!');
});
...
app.use('/api/dashboard_screen_time', dashboardScreenTimeRoutes);
app.use('/api/dashboard_app_time', dashboardAppTimeRoute);
...
connectToDatabase().then(() => {
  ...
  app.post('/add-child', async (req, res) => {
    ...
    try {
      const childCollection = db.collection('child_profiles');
      const childData = {
      ...
      const result = await childCollection.insertOne(childData);
      res.status(201).json({ message: 'Child added successfully', childId: result.insertedId });
    } catch (error) {
