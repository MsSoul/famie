...
router.post('/:childId', async (req, res) => {
    ...
    try {
        ...
        const enrichedTimeSlots = time_slots.map(slot => {
            const startTime = Date.parse(`1970-01-01T${slot.start_time}:00Z`);
            const endTime = Date.parse(`1970-01-01T${slot.end_time}:00Z`);
            const allowedTimeInSeconds = (endTime - startTime) / 1000; // Convert milliseconds to seconds
            
            return {
                ...slot,
                slot_identifier: new ObjectId(),
                allowed_time: allowedTimeInSeconds, // Set calculated allowed time
                is_allowed: slot.is_allowed === 'true' || slot.is_allowed === true,
            };
        });

        const updateResult = await timeManagementCollection.updateOne(
            { child_id: new ObjectId(childId) },
            { $set: { time_slots: enrichedTimeSlots } }, // Replace the time slots
            { upsert: true }
        );
